...
void solve()
{
    ...
    sort(home.begin(), home.end(),greater<int>());
    ...
    for(int i=0;i < m && ptr < n; i++){
        ...
        while(ptr < n && app[ptr]-k > home[i]){
            // cout<<"skip "<<app[ptr]<<endl;
            ptr++;
        }

        if( ptr < n && app[ptr] - k <= home[i] && home[i] <= app[ptr] + k){
            // cout<<"caught "<<app[ptr]<<endl;
            ans++;
            ptr++;
            continue;
        }
