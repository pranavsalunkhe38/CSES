// LC: 0000 Minimizing Coin
// Title: Minimizing Coin
// Difficulty: medium
// Date: 28 April 2026
...
int rec(vector<int>& v, int i, int x,vector<vector<int>>& dp)
{
        if(x == 0) return 0;
    ...
    int val = rec(v, i+1, x,dp);
    
    if(x - v[i] >= 0){
        int y = rec(v,i, x - v[i],dp);
        if(y != INT_MAX)
            val = min(val, 1+ y);
    }
...
for(int sum = 1; sum <= x ; sum++){
    for(int i = 0 ; i < n; i++){
       if (sum - v[i] >= 0 && dp[sum - v[i]] != INT_MAX) {
